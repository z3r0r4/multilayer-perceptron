...
public class Perceptron {
	...
	private Matrix y;
	ArrayList<Double> C, Cmean = new ArrayList<>();
	...
	public Perceptron(int n) {
		...
		System.out.println("generating node Gradients");
		...
		System.out.println("-------------------------------------------------");
		System.out.println("generating Bias and Weight Gradients foreach Layer except the last");
		...
		for (int i = 0; i < y.getRows(); i++)
			y.setData(0, i, 0); // filled with 0 for now atleast
		y.info(null);
	...
	public void APPLY() {
		for (int n = No_l - 2; n > 0; n--) {
			b[n].add(Matrix.negate(Gradient_b[n]));
			w[n].add(Matrix.negate(Gradient_w[n]));
		}
	}
	...
	public Matrix sigmoid_1(Matrix A) {
		Matrix B = new Matrix(A.getRows(), A.getColumns());
		for (int i = 0; i < A.getRows(); i++)
			for (int j = 0; j < A.getColumns(); j++)
	...
	public double sigmoid_1(double a) {
